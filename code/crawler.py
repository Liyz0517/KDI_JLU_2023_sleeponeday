...
def get_base_html(get):
    if get:
        ...
        file = open('base_html1.txt', 'w', encoding='utf8')
        file.write(res.text)
        print('主页源码爬取完成')
    else:
        text = open('base_html1.txt', encoding='utf8').read()
        print('主页源码获取完成')
        return text
...
def get_content():
    file = open('data_down/花卉大全.txt', 'r', encoding='utf8')
            ...
            while True:
                ...
                if content_page_html.status_code == 200:
                    ...
                    if len(end) <= 0:
                        break
                    index += 1
                    randomtime1 = random.uniform(5, 10)
                    sleep(randomtime1)
                    # if index == 2:
                    #     break
                else:
